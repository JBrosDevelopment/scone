...
use crate::{macros::Macros, error_handling::ErrorHandling, debug};
use serde::Serialize;

pub fn lex(error_handling: &mut ErrorHandling, macros: &mut Macros) -> Vec<Token> {
    let tokens = tokenize(error_handling, macros);
    ...
    while i < chars.len() {
                ...
                match directive {
                    ...
                    "if" => {
                        if macro_if_value.is_some() || macro_if_has_been_true.is_some() {
                            (line!(), "Unexpected `if` directive", "Nesting of `if` or `ifn` is not supported", &current_location);
                            continue;
                        }
                        match macros.parse_if_directive(message) {
                            Ok(v) => {
                            ...
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `if` directive", &e, &current_location);
                            }
                        }
                    } 
                    "ifn" => {
                        if macro_if_value.is_some() || macro_if_has_been_true.is_some() {
                            error(error_handling, line!(), "Unexpected `ifn` directive", "Nesting of `if` or `ifn` is not supported", &current_location);
                            continue;
                        }
                        match macros.parse_if_directive(message) {
                            Ok(v) => {
                            ...
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `ifn` directive", &e, &current_location);
                            }
                        }
                    }
                    "elif" => {
                        if macro_if_value.is_none() || macro_if_has_been_true.is_none() {
                            error(error_handling, line!(), "Unexpected `elif` directive", "Expected `if` or `elif` directive before `elif`", &current_location);
                            continue;
                        }
                        ...
                        match macros.parse_if_directive(message) {
                            ...
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `elif` directive", &e, &current_location);
                            }
                        }
                    }
                    "elifn" => {
                        if macro_if_value.is_none() || macro_if_has_been_true.is_none() {
                            error(error_handling, line!(), "Unexpected `elifn` directive", "Expected `if` or `elif` directive before `elifn`", &current_location);
                            continue;
                        }
                        if macro_if_has_been_true.unwrap() == true {
                            macro_if_value = Some(false);
                        ...
                        match macros.parse_if_directive(message) {
                            ...
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `elifn` directive", &e, &current_location);
                            }
                        }
                    }
                    "elifdef" => {
                        if macro_if_value.is_none() || macro_if_has_been_true.is_none() {
                            error(error_handling, line!(), "Unexpected `elifdef` directive", "Expected `if` or `elif` directive before `elifdef`", &current_location);
                            continue;
                        }
                        if macro_if_has_been_true.unwrap() == true {
                            macro_if_value = Some(false);
                        ...
                        match macros.parse_if_define_directive(message) {
                            Ok(v) => {
                                macro_if_value = Some(v);
                                if v {
                                    macro_if_has_been_true = Some(true);
                            ...
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `elifdef` directive", &e, &current_location);
                            }
                        }
                    }
                    "elifndef" => {
                        if macro_if_value.is_none() || macro_if_has_been_true.is_none() {
                            error(error_handling, line!(), "Unexpected `elifndef` directive", "Expected `if` or `elif` directive before `elifndef`", &current_location);
                            continue;
                        }
                        if macro_if_has_been_true.unwrap() == true {
                            macro_if_value = Some(false);
                        ...
                        match macros.parse_if_define_directive(message) {
                            ...
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `elifndef` directive", &e, &current_location);
                            }
                        }
                    }
                    "else" => {
                        if macro_if_value.is_none() ||  macro_if_has_been_true.is_none() {
                            error(error_handling, line!(), "Unexpected `else` directive", "Expected `if` or `elif` directive before `else`", &current_location);
                            continue;
                        }
                        if macro_if_has_been_true.unwrap() == true {
                            macro_if_value = Some(false);
                    ...
                    "ifdef" => {
                        if macro_if_value.is_some() || macro_if_has_been_true.is_some() {
                            error(error_handling, line!(), "Unexpected `ifdef` directive", "Nesting of `if` or `ifdef` is not supported", &current_location);
                            continue;
                        }
                        match macros.parse_if_define_directive(message) {
                            Ok(v) => {
                            ...
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `ifdef` directive", &e, &current_location);
                            }
                        }
                    }
                    "ifndef" => {
                        if macro_if_value.is_some() || macro_if_has_been_true.is_some() {
                            error(error_handling, line!(), "Unexpected `ifndef` directive", "Nesting of `if` or `ifndef` is not supported", &current_location);
                            continue;
                        }
                        match macros.parse_if_define_directive(message) {
                            Ok(v) => {
                    ...
                    "def" | "define" => {
                        match macros.parse_define_directive(message) {
                            Ok(_) => {},
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `def` directive", &e, &current_location);
                            }
                        }
                    }
                    "undef" | "undefine" => {
                        match macros.parse_undefine_directive(message) {
                            Ok(_) => {},
                            Err(e) => {
                                error(error_handling, line!(), "Error parsing `undef` directive", &e, &current_location);
                            }
                        }
                    }
                    _ => {
                        error(error_handling, line!(), "Unknown shebang directive", "`if`, `ifn`, `elif`, `elifn`, `else`, `endif`, `ifdef`, `ifndef`, `def`, `undef` is supported in shebangs", &current_location);
                    }
                }
        ...
        if matches!(c, ' ' | '\t' | '\r' | '\n' | '#') {
            ...
            while i < chars.len() {
                if char::is_numeric(chars[i]) {
                    if binary && chars[i] != '0' && chars[i] != '1' {
                        error(error_handling, line!(), "invalid binary number", "Binary prefix `0b` must be followed by only `0` or `1`", &current_location);
                        break;
                    }
                    number.push(chars[i]);
                    i += 1;
                    ...
                    if chars[i] == 'a' || chars[i] == 'b' || chars[i] == 'c' || chars[i] == 'd' || chars[i] == 'e' || chars[i] == 'f' {
                        ...
                        error(error_handling, line!(), "invalid hex number", "Hex prefix `0x` must be followed by only '0' to '9' and `a` to `f`", &current_location);
                        break;
                    }
                } else {
                    break;
            ...
            if chars.get(i + 1) == Some(&'+') {
                ...
                current_location.advance(2);
                i += 1;
                error(error_handling, line!(), "Scone has no increment operator", "not a valid operator, use `+=` instead", &current_location);
            }
            else if chars.get(i + 1) == Some(&'=') {
                last_was_negatable_ability = 3;
                current_location.advance(2);
            ...
            if chars.get(i + 1) == Some(&'-') {
                ...
                current_location.advance(2);
                i += 1;
                error(error_handling, line!(), "Scone has no decrement operator", "not a valid operator, use `-=` instead", &current_location);
            }
            else if chars.get(i + 1) == Some(&'>') {
                current_location.advance(2);
            ...
            if char.len() != 1 {
                error(error_handling, line!(), "Invalid escape sequence", "", &current_location);
                i += 2;
                continue;
            }
            ...
            error(error_handling, line!(), format!("Unknown character: {}", c).as_str(), "", &current_location);
            continue;
        }
        i += 1;
        last_was_negatable_ability -= 1;
